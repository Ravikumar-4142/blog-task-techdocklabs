              ...
              <div class="bg-white shadow-md rounded my-6 p-5">
                <form method="POST" action="{{ route('admin.posts.update',$post->id)}}">
                    ...
                    <div class="relative inline-flex">
                      <svg class="w-2 h-2 absolute top-0 right-0 m-4 pointer-events-none" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <select
                          ...
                          <option value="{{ $value->id }}" @if($post->category_id == $value->id) selected @endif>{{ $value->title }}</option>
                          @endforeach
                        </select>
                      </div>
                      @error('category_id')
                  ...
                  <div class="flex flex-col space-y-2">
                    <label for="title" class="text-gray-700 select-none font-medium"><span class="text-red-600">*</span>Title</label>
